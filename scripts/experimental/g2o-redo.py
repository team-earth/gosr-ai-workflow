...
setup_logging("g2o-redo.log", backup_count=5)
...
def insert_causative4(node, future_picture):
    ...
    msg_text = f'The future picture "{future_picture}" has an obstacle "{obstacle}". ' + \
      'Produce a list of the contributing factors to this obstacle in JSON format. ' + \
      'Each subobstacle in the list is a dict with structure: ' + \
      '{"obstacle": { "data": {"title":"...", "description":"..."}}}.'
    logger.info(msg_text)
    # text = robust_call(model, messages)
...
def main():
    ...
    # tree.create_node(data=config["root_node_name"], identifier="goal", tag="root")
    ...
    utils.load_tree(os.path.join(path, "o-orig-old.json"))
    ...
    for n in leaf_list:
        ...
        print_tree("ROOT")
        count += 1
        print(f"{datetime.now().isoformat()} {count}/{len(leaf_list)} {100*count/len(leaf_list):.3g}%", n.data)
