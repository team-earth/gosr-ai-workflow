...
def main():
    "Main functionality"
    # global config, path
    global eval_categorization, score_categorization, global_resources_list, cache_dirty

    eval_categorization = config["solution_categories"]
    score_categorization = config["score_categorization"]
    # country = config["country"]
    # max_resource_loops = config.get("max_resource_loops", max_resource_loops)

    cache_path = os.path.join(path, "cache-assess.json")
    ...
    load_resources(os.path.join(path, "resources-score.json"))
    flat_df = json_flatten(global_resources_list)
    df = pd.DataFrame(flat_df)
    print(df.head())
    df.to_csv(os.path.join(path, 'r-scale.csv'))
...
if __name__ == "__main__":
    ...
    log_filename = os.path.join(path, "assess.log")
