...
def validate_score(n):
    x = {
        ...
        "Feasibility": {
            "TechnicalViability": 9,
            "FinancialPracticality": 7,
            "RegulatoryCompliance": 8,
            "PoliticalViability": 7,
        },
        "Innovativeness": {
            "Originality": 5,
            "ApplicationOfTechnology": 7,
            "ProblemSolvingApproach": 6,
        },
        "Scalability": {"Adaptability": 7, "GrowthPotential": 7, "Replicability": 8},
        "CommunityEngagement": {
            "Inclusivity": 8,
            "ParticipationMechanisms": 6,
            "AwarenessAndEducation": 7,
        },
        "Sustainability": {
            "EnvironmentalConsiderations": 7,
            "EconomicSustainability": 6,
            "SocialSustainability": 8,
        },
    }

    if compare_dict_structure(n,x):
        raise ValueError("Dict does not comply")


def add_profile(node):
    d = {"program": node["program"], "description": node["description"]}
    count = 0
    found = False
    use_cache = True
...
def load_resources():
    ...
    file_path = os.path.join(path, "resources-eval.json")
...
def main():
    "Main functionality"
    # global config, path
    global evaluation_categories, global_resources_list, cache_dirty

    evaluation_categories = config["score_categorization"]
    # country = config["country"]
    # max_resource_loops = config.get("max_resource_loops", max_resource_loops)

    cache_path = os.path.join(path, "cache-score.json")
    ...
    for r in global_resources_list:
        ...
        if "dup" in r:
            print(
            f"D {datetime.now().isoformat()} {count}/{total_count} {100*count/total_count:.3g}%")
            continue
        
        print(
            f"{datetime.now().isoformat()} {count}/{total_count} {100*count/total_count:.3g}%")
        add_profile(r)
        # print_tree("root")
        # save_tree()
...
if __name__ == "__main__":
    ...
    log_filename = os.path.join(path, "r-score.log")
